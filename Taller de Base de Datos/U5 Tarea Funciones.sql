...
*/
create function fn_empleadoTerritorio(@emp int)
...
*/
create function dbo.fn_productosCategorias()
...
/*
6.- FUNCION DE TABLA DE MULTISENTENCIA QUE RECIBA UN AÑO COMO PARAMETRO DE ENTRADA, QUE REGRESE UNA TABLA CON DOS COLUMNAS:
DIA DE LA SEMANA, FOLIOS QUE SE VENDIERON ESE DÍA DE SEMANA. NOTA, DEBE MOSTRAR TODOS LOS DIAS DE LA SEMANA AUNQUE NO SE HAYAN REALIZADO ORDENES.
*/
create function dbo.fn_diaFolios()
returns @folios table(diasemana datetime(20), numfolio int)
as
begin
	declare @diasemana datetime, @numfolio int

	select @numfolio = min(orderid) from orders
	while @numfolio is not null
	begin

		select @numfolio = orderid from orders where orderid = @numfolio
		select @diasemana = day(orderdate) from orders where orderid = @numfolio
	end
return
end

--ejecucion
select * from dbo.fn_dia_Folios()